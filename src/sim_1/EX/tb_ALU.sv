...
module tb_ALU;
    ...
    task automatic check(input string name, input logic [DATA_WIDTH-1:0] expected_result,
    ...
    initial begin
        ...
        // SRA: -8 >>> 2 = -2
        i_operand_a = 32'hFFFF_FFF8;
        i_operand_b = 32'd2;
        i_ALUCtrl   = 4'b0111;
        check("SRA -8>>>2", 32'hFFFF_FFFE, 1'b0);

        // OR: 0xA0 | 0x0F = 0xAF
        i_operand_a = 32'h0000_00A0;
        i_operand_b = 32'h0000_000F;
        i_ALUCtrl   = 4'b1000;
        check("OR", 32'h0000_00AF, 1'b0);

        // AND: 0xFF & 0x0F = 0x0F
        i_operand_a = 32'h0000_00FF;
        i_operand_b = 32'h0000_000F;
        i_ALUCtrl   = 4'b1001;
