...
module tb_ControlUnit;
    ...
    ControlUnit #(.OPCODE_WIDTH(7)) DUT (
        .i_opcode   (opcode),
        .o_RegWrite (RegWrite),
        .o_ALUSrc   (ALUSrc),
        .o_ALUOp    (ALUOp),
    ...
    task automatic check(
        ...
        if (RegWrite !== exp_RegWrite || ALUSrc !== exp_ALUSrc || ALUOp !== exp_ALUOp ||
            ...
            $display("FAIL [%s] opcode=%07b | got RegWrite=%b ALUSrc=%b ALUOp=%02b MemRead=%b MemWrite=%b MemToReg=%b Branch=%b Jump=%b ImmSrc=%03b",
                name, opcode,
                RegWrite, ALUSrc, ALUOp, MemRead, MemWrite, MemToReg, Branch, Jump, ImmSrc);
            $display("     expected         RegWrite=%b ALUSrc=%b ALUOp=%02b MemRead=%b MemWrite=%b MemToReg=%b Branch=%b Jump=%b ImmSrc=%03b",
                exp_RegWrite, exp_ALUSrc, exp_ALUOp, exp_MemRead, exp_MemWrite, exp_MemToReg, exp_Branch, exp_Jump, exp_ImmSrc);
            fail_count++;
        end else begin
            $display("PASS [%s]", name);
            pass_count++;
